...
export const Input = React.forwardRef<HTMLInputElement, React.InputHTMLAttributes<HTMLInputElement>>(({ className = '', ...props }, ref) => (
  <input 
    ref={ref}
    className={`w-full rounded-lg px-4 py-2.5 text-base placeholder-gray-400 bg-gray-50 border-2 border-gray-200 text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500 transition-all ${className}`} 
    {...props} 
  />
));
Input.displayName = 'Input';

export const Badge: React.FC<{ children: React.ReactNode, color?: string, className?: string, variant?: 'success' | 'neutral' }> = ({ children, color, className = '', variant = 'neutral' }) => {
  let finalColor = color;
  if (!finalColor) {
    if (variant === 'success') finalColor = 'bg-green-150 text-green-800 bg-green-50 border border-green-100';
    else finalColor = 'bg-gray-100 text-gray-800';
  }
  return (
    <span className={`${finalColor} text-xs px-2 py-1 rounded-md font-bold ${className}`}>
      {children}
    </span>
  );
};
